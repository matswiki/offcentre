	...
	return {
		...
		startMoving : function (divObject, container, evt) {
			...
			document.onmousemove = function (evt) {
				...
				if (aY + eHe > cHe)
					aY = cHe - eHe;
				dragQueen.move(divObject, aX, aY);
			}
		},
...
function drag(ev) {
	dragItem = document.getElementById(ev.target.id);
	xElem = dragItem.offsetLeft - ev.pageX;
	yElem = dragItem.offsetTop - ev.pageY - 10;
}

function allowDrop(ev) {
	ev.preventDefault();
	dragItem.style.left = (ev.pageX + xElem) + "px";
	dragItem.style.top = (ev.pageY + yElem) + "px";
}
...
function generateXsdTypes(minify) {
  ...
  output += "<xs:element name=\""+ lowerCaseFirstLetter(recordName) +"\" type=\""+recordName + "Type\"/>";
    ...
    /*
		if (rowIndex == 1) {
			...
			output += lowerCaseFirstLetter(recordName) + "Id";
			output += "\" type=\"";
			output += capitalizeFirstLetter(recordName) + "IdType";
			output += "\" minOccurs=\"0\" />";
		}
    */
		var row = table.rows[rowIndex];
  ...
  /*
	for (var rowIndex = 1; rowIndex < rowCount; rowIndex++) {
		if (rowIndex == 1) {
			output += "<xs:simpleType name=\"";
			output += capitalizeFirstLetter(recordName) + "IdType";
			output += "\">";
			output += "<xs:restriction base=\"xs:decimal\">";
			output += "</xs:restriction>";
			output += "</xs:simpleType>";
		}
		var row = table.rows[rowIndex];
    var type = getXsdBaseType(row.cells[1].innerHTML);
		output += "<xs:simpleType name=\"";
		output += capitalizeFirstLetter(row.cells[0].innerHTML)+"Type";
		output += "\">";
		output += "<xs:restriction base=\"xs:" + type + "\">";
    if ( type != "decimal" && type != "date") {
      output += "<xs:minLength value=\"0\"/>";
      output += "<xs:maxLength value=\"" + row.cells[3].innerHTML + "\"/>";
    }
		output += "</xs:restriction>";
